...
# Verifica se cpupower está instalado
if ! command -v cpupower >/dev/null || ! command -v kitty >/dev/null; then
  echo "cpupower é kitty não encontrados."
  ...
  if [[ "$resposta" =~ ^[Ss]$ ]]; then
    ...
    echo "cpupower é kitty necessário para o script funcionar. Abortando."
    exit 1
  fi
fi
...
# Se quiser remover os prints depois de configurado:
VERBOSE=false

if ! grep -q "class:gov-cpu" "$HYPRCONF"; then
  $VERBOSE && echo "Adicionando regras em hyprland.conf para gov-cpu..."

  if [ ! -f "$BACKUP" ]; then
    cp "$HYPRCONF" "$BACKUP"
    $VERBOSE && echo "Backup criado em $BACKUP"
  fi
...
windowrulev2 = size 600 200, class:gov-cpu
EOF

  echo "Hyprland.conf atualizado"
  echo "Regras adicionadas. Reinicie ou recarregue o Hyprland."
fi
